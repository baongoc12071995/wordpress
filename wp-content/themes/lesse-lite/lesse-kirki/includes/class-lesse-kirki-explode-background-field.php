...
if ( ! class_exists( 'Lesse_Kirki_Explode_Background_Field' ) ) {

	/**
	 * Explodes background fields and creates sub-fields for it.
	 */
	class Lesse_Kirki_Explode_Background_Field {
		...
		public static function explode( $field ) {
			$i18n    = Lesse_Kirki_l10n::get_strings( $field['lessekirki_config'] );
				...
				switch ( $key ) {
					case 'color':
						...
						if ( isset( $field['default']['opacity'] ) && false === strpos( $value, 'rgb' ) ) {
							$value = Lesse_Kirki_Color::get_rgba( $value, $field['default']['opacity'] );
						}
						$sanitize_callback = array( 'Lesse_Kirki_Sanitize_Values', 'color' );
						break;
					case 'image':
						$type = 'image';
