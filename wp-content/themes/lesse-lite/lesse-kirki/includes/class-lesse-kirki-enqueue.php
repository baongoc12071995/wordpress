...
if ( ! class_exists( 'Lesse_Kirki_Enqueue' ) ) {

	/**
	 * Enqueues JS & CSS assets
	 */
	class Lesse_Kirki_Enqueue {
		...
		public function customize_controls_l10n() {
			...
			wp_register_script( 'lessekirki-l10n', trailingslashit( Lesse_Kirki::$url ) . 'assets/js/l10n.js' );
			...
			foreach ( $configs as $id => $args ) {
				$l10n[ $id ] = Lesse_Kirki_l10n::get_strings( $id );
			}

			wp_localize_script( 'lessekirki-l10n', 'lessekirkiL10n', $l10n );
			wp_enqueue_script( 'lessekirki-l10n' );
		...
		public function customize_controls_enqueue_scripts() {
			...
			if ( $has_tooltips ) {
				wp_enqueue_script( 'lessekirki-tooltip', trailingslashit( Lesse_Kirki::$url ) . 'assets/js/tooltip.js', array( 'jquery', 'customize-controls', 'jquery-ui-tooltip' ) );
			}

			// Enqueue the reset script.
			wp_enqueue_script( 'lessekirki-reset', trailingslashit( Lesse_Kirki::$url ) . 'assets/js/reset.js', array( 'jquery', 'lessekirki-set-setting-value' ) );

			// Register lessekirki-functions.
			wp_register_script( 'lessekirki-set-setting-value', trailingslashit( Lesse_Kirki::$url ) . 'assets/js/functions/set-setting-value.js' );
			wp_register_script( 'lessekirki-validate-css-value', trailingslashit( Lesse_Kirki::$url ) . 'assets/js/functions/validate-css-value.js' );

			// Register serialize.js.
			wp_register_script( 'serialize-js', trailingslashit( Lesse_Kirki::$url ) . 'assets/js/vendor/serialize.js' );

			// Register the color-alpha picker.
			wp_enqueue_style( 'wp-color-picker' );
			wp_register_script( 'wp-color-picker-alpha', trailingslashit( Lesse_Kirki::$url ) . 'assets/js/vendor/wp-color-picker-alpha.js', array( 'wp-color-picker' ), '1.2', true );

			// Register the jquery-ui-spinner.
			wp_register_script( 'jquery-ui-spinner', trailingslashit( Lesse_Kirki::$url ) . 'assets/js/vendor/jquery-ui-spinner', array( 'jquery', 'jquery-ui-core', 'jquery-ui-button' ) );

			// Register codemirror.
			wp_register_script( 'codemirror', trailingslashit( Lesse_Kirki::$url ) . 'codemirror/lib/codemirror.js', array( 'jquery' ) );

			// Register selectize.
			wp_register_script( 'selectize', trailingslashit( Lesse_Kirki::$url ) . 'assets/js/vendor/selectize.js', array( 'jquery' ) );
			...
			$scripts = array(
				...
				'dimension'       => array( 'jquery', 'customize-base', 'lessekirki-validate-css-value' ),
				'dropdown-pages'  => array( 'jquery', 'customize-base', 'selectize' ),
				'editor'          => array( 'jquery', 'customize-base', 'lessekirki-l10n' ),
				...
				'preset'          => array( 'jquery', 'customize-base', 'selectize', 'lessekirki-set-setting-value' ),
				'radio-buttonset' => array( 'jquery', 'customize-base' ),
				'radio-image'     => array( 'jquery', 'customize-base' ),
				'radio'           => array( 'jquery', 'customize-base' ),
				'repeater'        => array( 'jquery', 'customize-base', 'jquery-ui-core', 'jquery-ui-sortable', 'lessekirki-l10n' ),
				...
				'spacing'         => array( 'jquery', 'customize-base', 'lessekirki-validate-css-value' ),
			...
			foreach ( $scripts as $id => $dependencies ) {
				wp_register_script( 'lessekirki-' . $id, trailingslashit( Lesse_Kirki::$url ) . 'assets/js/controls/' . $id . '.js', $dependencies, false, true );
			}

			// Add fonts to our JS objects.
			$google_fonts   = Lesse_Kirki_Fonts::get_google_fonts();
			$standard_fonts = Lesse_Kirki_Fonts::get_standard_fonts();
			$all_variants   = Lesse_Kirki_Fonts::get_all_variants();
			$all_subsets    = Lesse_Kirki_Fonts::get_google_font_subsets();
			...
			wp_localize_script( 'lessekirki-typography', 'lessekirkiAllFonts', $final );
		}
		...
		public function branding() {
			...
			if ( ! empty( $vars['logoImage'] ) || ! empty( $vars['description'] ) ) {
				wp_register_script( 'lessekirki-branding', Lesse_Kirki::$url . '/assets/js/branding.js' );
				wp_localize_script( 'lessekirki-branding', 'lessekirkiBranding', $vars );
				wp_enqueue_script( 'lessekirki-branding' );
			}
		}
		...
		public function postmessage() {
			wp_enqueue_script( 'lessekirki_auto_postmessage', trailingslashit( Lesse_Kirki::$url ) . 'assets/js/postmessage.js', array( 'customize-preview' ), false, true );
			...
			wp_localize_script( 'lessekirki_auto_postmessage', 'jsvars', $js_vars_fields );
		}
	}
}
