...
if ( ! class_exists( 'Lesse_Kirki_Controls_Repeater_Control' ) ) {

	/**
	 * Repeater control
	 */
	class Lesse_Kirki_Controls_Repeater_Control extends Lesse_Kirki_Customize_Control {
			...
			if ( is_array( $this->fields ) && ! empty( $this->fields ) ) {
				...
				foreach ( $this->fields as $field ) {
					if ( isset( $field['type'] ) && 'dropdown-pages' === $field['type'] ) {
						wp_enqueue_script( 'lessekirki-dropdown-pages' );
						break;
					}
					if ( isset( $field['type'] ) && 'autocomplete' === $field['type'] ) {
					    wp_enqueue_script( 'jquery-ui-autocomplete' );
					}
				}
			}

			wp_enqueue_script( 'lessekirki-repeater' );
			wp_localize_script( 'lessekirki-repeater', 'lesseKirkiparams', array(
				'nonce' => wp_create_nonce( 'lesse-autocomplete-ajax' ),
			) );
		}
		...
		protected function render_content() {
			?>
			<?php $l10n = Lesse_Kirki_l10n::get_strings( $this->lessekirki_config ); ?>
			<?php if ( '' !== $this->tooltip ) : ?>
		...
		public function repeater_js_template() {
			$l10n = Lesse_Kirki_l10n::get_strings( $this->lessekirki_config );
			$add_new_post_link = site_url( '/wp-admin/post-new.php' );
			?>
					...
					<div class="repeater-row-content">
						<# _.each( data, function( field, i ) { #>
							...
							<div class="repeater-field repeater-field-{{{ field.type }}}">
								<?php //Supernova Themes: Added support for number ?>
										...
										<# if ( field.label ) { #>
											...
											<# } #>
												<# if ( field.description ) { #>
													...
													<# } #>
													<input type="text" class="lesse-autocomplete" name="" value="{{{ field.default }}}" data-field="{{{ field.id }}}">
													<span class="spinner"></span>
													<a href="<?php echo esc_url( $add_new_post_link ); ?>" target="_blank" class="lesse-repeat-add-new-post"><span class="dashicons dashicons-external"></span><?php esc_html_e( "Create New Post", 'lesse-lite' ) ?></a>
													<span class="description customize-control-description"><i><?php esc_html_e( 'You do not need to refresh the customizer after creating a new post. Your new post will automatically become available in the search results.', 'lesse-lite' ) ?></i></span>
									</label>
								<# } else if ( 'text' === field.type || 'url' === field.type || 'email' === field.type || 'tel' === field.type || 'date' === field.type || 'number' === field.type ) { #>
									...
									<label>
										...
										<# if ( 'number' === field.type ) { #>
											<input type="{{field.type}}" name="" min="0" max="500" value="{{{ field.default }}}" data-field="{{{ field.id }}}">
										<# } else { #>
								...
								<# } else if ( 'color' === field.type ) { #>
							        ...
							        <label>
							            ...
							            <# if ( field.description ) { #>
							                <span class="description customize-control-description">{{{ field.description }}}</span>
							            <# } #>
							            <input class="color-picker-hex" type="text" maxlength="7" placeholder="<?php echo esc_attr( $l10n['hex-value'] ); ?>"  value="{{{ field.default }}}" data-field="{{{ field.id }}}" {{ defaultValue }} />
								...
								<# } else if ( field.type === 'image' || field.type === 'cropped_image' ) { #>
									...
									<figure class="lessekirki-image-attachment" data-placeholder="<?php echo esc_attr( $l10n['no-image-selected'] ); ?>" >
										<# if ( field.default ) { #>
									...
									<div class="actions">
										<button type="button" class="button remove-button<# if ( ! field.default ) { #> hidden<# } #>"><?php echo esc_attr( $l10n['remove'] ); ?></button>
										<button type="button" class="button upload-button" data-label=" <?php echo esc_attr( $l10n['add-image'] ); ?>" data-alt-label="<?php echo esc_attr( $l10n['change-image'] ); ?>" >
											<# if ( field.default ) { #>
												<?php echo esc_attr( $l10n['change-image'] ); ?>
											<# } else { #>
												<?php echo esc_attr( $l10n['add-image'] ); ?>
											<# } #>
										</button>
										<# if ( field.default.id ) { #>
								...
								<# } else if ( field.type === 'upload' ) { #>
									...
									<figure class="lessekirki-file-attachment" data-placeholder="<?php echo esc_attr( $l10n['no-file-selected'] ); ?>" >
										<# if ( field.default ) { #>
										...
										<# } else { #>
											<?php echo esc_attr( $l10n['no-file-selected'] ); ?>
										<# } #>
									</figure>

									<div class="actions">
										<button type="button" class="button remove-button<# if ( ! field.default ) { #> hidden<# } #>"></button>
										<button type="button" class="button upload-button" data-label="<?php echo esc_attr( $l10n['add-file'] ); ?>" data-alt-label="<?php echo esc_attr( $l10n['change-file'] ); ?>" >
											<# if ( field.default ) { #>
												<?php echo esc_attr( $l10n['change-file'] ); ?>
											<# } else { #>
												<?php echo esc_attr( $l10n['add-file'] ); ?>
											<# } #>
										</button>
										<# if ( field.default.id ) { #>
											<input type="hidden" class="hidden-field" value="{{{ field.default.id }}}" data-field="{{{ field.id }}}" >
						...
						<# }); #>
						<button type="button" class="button-link repeater-row-remove">Remove</button>
					</div>
				</li>
			</script>
			<?php
