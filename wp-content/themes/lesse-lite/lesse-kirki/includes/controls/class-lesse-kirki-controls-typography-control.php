...
if ( ! class_exists( 'Lesse_Kirki_Controls_Typography_Control' ) ) {

	/**
	 * Typography control.
	 */
	class Lesse_Kirki_Controls_Typography_Control extends Lesse_Kirki_Customize_Control {
		 ...
		 * @var string
		 */
		public $type = 'lessekirki-typography';
		...
		public function enqueue() {
			wp_enqueue_script( 'lessekirki-typography' );
		}
		...
		public function to_json() {
			...
			$this->json['show_variants'] = ( true === Lesse_Kirki_Fonts_Google::$force_load_all_variants ) ? false : true;
			$this->json['show_subsets']  = ( true === Lesse_Kirki_Fonts_Google::$force_load_all_subsets ) ? false : true;
		}
			...
			<div class="wrapper">
				...
				<# if ( data.default['font-family'] ) { #>
					...
					<div class="font-family">
						...
						<select id="lessekirki-typography-font-family-{{{ data.id }}}" placeholder="{{ data.l10n['select-font-family'] }}"></select>
					</div>
					<# if ( true === data.show_variants || false !== data.default.variant ) { #>
						<div class="variant hide-on-standard-fonts lessekirki-variant-wrapper">
							<h5>{{ data.l10n['variant'] }}</h5>
							<select class="variant" id="lessekirki-typography-variant-{{{ data.id }}}"></select>
						</div>
					<# } #>
					<# if ( true === data.show_subsets ) { #>
						<div class="subsets hide-on-standard-fonts lessekirki-subsets-wrapper">
							<h5>{{ data.l10n['subsets'] }}</h5>
							<select class="subset" id="lessekirki-typography-subsets-{{{ data.id }}}"></select>
						</div>
					<# } #>
				<# } #>
				...
				<# if ( data.default['text-transform'] ) { #>
					<div class="text-transform">
						...
						<select id="lessekirki-typography-text-transform-{{{ data.id }}}">
				...
				<# if ( data.default['color'] ) { #>
					<div class="color">
						...
						<input type="text" data-palette="{{ data.palette }}" data-default-color="{{ data.default['color'] }}" value="{{ data.value['color'] }}" class="lessekirki-color-control color-picker" {{{ data.link }}} />
					</div>
				<# } #>
			</div>
			<?php
