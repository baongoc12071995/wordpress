...
if ( ! class_exists( 'Lesse_Kirki_Fonts_Google' ) ) {

	/**
	 * Manages the way Google Fonts are enqueued.
	 */
	final class Lesse_Kirki_Fonts_Google {

		/**
		 * The Lesse_Kirki_Fonts_Google instance.
		...
		private function __construct() {
			...
			$this->google_fonts = Lesse_Kirki_Fonts::get_google_fonts();
		 ...
		 *
		 * @return object Lesse_Kirki_Fonts_Google
		 */
		public static function get_instance() {
			if ( null === self::$instance ) {
				self::$instance = new Lesse_Kirki_Fonts_Google();
			}
		...
		public function enqueue() {
			...
			if ( '' !== $this->link ) {
				wp_enqueue_style( 'lessekirki_google_fonts', $this->link, array(), null );
			}
		}
		...
		private function loop_fields() {
			foreach ( Lesse_Kirki::$fields as $field_id => $args ) {
		...
		private function generate_google_font( $args ) {
			...
			if ( isset( $args['type'] ) && 'lessekirki-typography' === $args['type'] ) {

				// Get the value.
				$value = Lesse_Kirki_Values::get_sanitized_field_value( $args );
				...
				if ( isset( $value['subsets'] ) ) {
					...
					// Add the subset directly to the array of subsets in the Lesse_Kirki_GoogleFonts_Manager object.
