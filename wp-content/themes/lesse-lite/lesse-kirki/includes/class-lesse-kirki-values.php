...
if ( ! class_exists( 'Lesse_Kirki_Values' ) ) {

	/**
	 * Wrapper class for static methods.
	 */
	class Lesse_Kirki_Values {
		 ...
		 * @static
		 * @access public
		 * @param string $config_id The configuration ID. @see Lesse_Kirki_Config.
		...
		public static function get_value( $config_id = '', $field_id = '' ) {
			...
			if ( ! isset( Lesse_Kirki::$config[ $config_id ] ) ) {
				$config_id = 'global';
			}

			if ( 'theme_mod' === Lesse_Kirki::$config[ $config_id ]['option_type'] ) {

				// We're using theme_mods so just get the value using get_theme_mod.
				$default_value = null;
				if ( isset( Lesse_Kirki::$fields[ $field_id ] ) && isset( Lesse_Kirki::$fields[ $field_id ]['default'] ) ) {
					$default_value = Lesse_Kirki::$fields[ $field_id ]['default'];
				}
				$value = get_theme_mod( $field_id, $default_value );

				// If the field is a background field, then get the sub-fields
				// and return an array of the values.
				if ( isset( Lesse_Kirki::$fields[ $field_id ] ) && isset( Lesse_Kirki::$fields[ $field_id ]['type'] ) && 'background' === Lesse_Kirki::$fields[ $field_id ]['type'] ) {
					$value = array();
			...
			} elseif ( 'option' === Lesse_Kirki::$config[ $config_id ]['option_type'] ) {

				// We're using options.
				if ( '' !== Lesse_Kirki::$config[ $config_id ]['option_name'] ) {

					// Options are serialized as a single option in the db.
					// We'll have to get the option and then get the item from the array.
					$options = get_option( Lesse_Kirki::$config[ $config_id ]['option_name'] );

					if ( ! isset( Lesse_Kirki::$fields[ $field_id ] ) && isset( Lesse_Kirki::$fields[ Lesse_Kirki::$config[ $config_id ]['option_name'] . '[' . $field_id . ']' ] ) ) {
						$field_id = Lesse_Kirki::$config[ $config_id ]['option_name'] . '[' . $field_id . ']';
					}
					$setting_modified = str_replace( ']', '', str_replace( Lesse_Kirki::$config[ $config_id ]['option_name'] . '[', '', $field_id ) );

					// If this is a background field, get the individual sub-fields and return an array.
					if ( 'background' === Lesse_Kirki::$fields[ $field_id ]['type'] ) {
						$value = array();

						foreach ( Lesse_Kirki::$fields[ $field_id ]['default'] as $property => $property_default ) {
						...
						$value = ( isset( $options[ $setting_modified ] ) ) ? $options[ $setting_modified ] : Lesse_Kirki::$fields[ $field_id ]['default'];
						$value = maybe_unserialize( $value );
					}
				} else {

					// Each option separately saved in the db.
					$value = get_option( $field_id, Lesse_Kirki::$fields[ $field_id ]['default'] );

					// If the field is a background field, then get the sub-fields.
					// and return an array of the values.
					if ( 'background' === Lesse_Kirki::$fields[ $field_id ]['type'] ) {
						$value = array();
						foreach ( Lesse_Kirki::$fields[ $field_id ]['default'] as $property_key => $property_default ) {
			...
			return apply_filters( 'lessekirki/values/get_value', $value, $field_id );
