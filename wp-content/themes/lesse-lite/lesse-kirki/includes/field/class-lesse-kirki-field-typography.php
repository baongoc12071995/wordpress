...
if ( ! class_exists( 'Lesse_Kirki_Field_Typography' ) ) {

	/**
	 * Field overrides.
	 */
	class Lesse_Kirki_Field_Typography extends Lesse_Kirki_Field {
		...
		protected function set_type() {
			...
			$this->type = 'lessekirki-typography';
		...
		public static function sanitize( $value ) {
			...
			if ( isset( $value['variant'] ) || isset( $value['font-weight'] ) ) {
				...
				$valid_variants = Lesse_Kirki_Fonts::get_all_variants();
			...
			if ( isset( $value['subsets'] ) ) {
				$valid_subsets = Lesse_Kirki_Fonts::get_google_font_subsets();
				$subsets_ok = array();
			...
			if ( isset( $value['font-size'] ) && ! empty( $value['font-size'] ) ) {
				$value['font-size'] = Lesse_Kirki_Sanitize_Values::css_dimension( $value['font-size'] );
			...
			if ( isset( $value['line-height'] ) && ! empty( $value['line-height'] ) ) {
				$value['line-height'] = Lesse_Kirki_Sanitize_Values::css_dimension( $value['line-height'] );
			}

			// Sanitize the letter-spacing.
			if ( isset( $value['letter-spacing'] ) && ! empty( $value['letter-spacing'] ) ) {
				$value['letter-spacing'] = Lesse_Kirki_Sanitize_Values::css_dimension( $value['letter-spacing'] );
