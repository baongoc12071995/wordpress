...
if ( ! class_exists( 'Lesse_Kirki_Field' ) ) {

	/**
	 * Please do not use this class directly.
	 * You should instead extend it per-field-type.
	 */
	class Lesse_Kirki_Field {

		/**
		 * The ID of the lessekirki_config we're using.
		 *
		 * @see Lesse_Kirki_Config
		 * @access protected
		 * @var string
		 */
		protected $lessekirki_config = 'global';
		 ...
		 * @var string
		 */
		protected $type = 'lessekirki-generic';
		 ...
		 *
		 * @see https://lessekirki.org/docs/arguments/output
		 * @access protected
		 * @var array
		 */
		 ...
		 *
		 * @see https://lessekirki.org/docs/arguments/js_vars
		 * @access protected
		 * @var array
		 */
		 ...
		 *
		 * @see https://lessekirki.org/docs/arguments/variables
		 * @access protected
		 * @var array
		 */
		 ...
		 * Then it adds the field to Lesse_Kirki::$fields.
		 *
		 * @access public
		 * @param string $config_id    The ID of the config we want to use.
		 *                             Defaults to "global".
		 *                             Configs are handled by the Lesse_Kirki_Config class.
		...
		public function __construct( $config_id = 'global', $args = array() ) {
			...
			if ( isset( $args['setting'] ) && ! empty( $args['setting'] ) && ( ! isset( $args['settings'] ) || empty( $args['settings'] ) ) ) {
				...
				unset( $args['setting'] );
				error_log( 'Lesse_Kirki: Typo found in field ' . $args['settings'] . ' ("setting" instead of "settings").' );
			}

			if ( is_string( $config_id ) ) {
				$args['lessekirki_config'] = $config_id;
			}
			...
			if ( is_array( $config_id ) && empty( $args ) ) {
				$args = $config_id;
				$this->lessekirki_config = 'global';
			}
			$this->lessekirki_config = trim( esc_attr( $config_id ) );
			if ( '' === $config_id ) {
				$this->lessekirki_config = 'global';
			}
			...
			$config_defaults = ( isset( Lesse_Kirki::$config['global'] ) ) ? Lesse_Kirki::$config['global'] : array();
			if ( 'global' !== $this->lessekirki_config && isset( Lesse_Kirki::$config[ $this->lessekirki_config ] ) ) {
				$config_defaults = Lesse_Kirki::$config[ $this->lessekirki_config ];
			}
			...
			$whitelisted = apply_filters( 'lessekirki/' . $this->lessekirki_config . '/fields/properties_whitelist', array(
		...
		protected function set_field( $whitelisted_properties = array() ) {
			...
			Lesse_Kirki::$fields[ $this->settings ] = $args;

			if ( 'background' === $this->type ) {
				// Build the background fields.
				Lesse_Kirki::$fields = Lesse_Kirki_Explode_Background_Field::process_fields( Lesse_Kirki::$fields );
			}
		...
		/**
		 * Escape $lessekirki_config.
		 *
		 * @access protected
		 */
		protected function set_lessekirki_config() {

			$this->lessekirki_config = esc_attr( $this->lessekirki_config );
		...
		protected function set_settings() {
			...
			if ( ! is_array( $this->settings ) ) {
				$this->settings = array( 'lessekirki_placeholder_setting' => $this->settings );
			}
			$settings = array();
			...
			if ( isset( $this->settings['lessekirki_placeholder_setting'] ) ) {
				$this->settings = $this->settings['lessekirki_placeholder_setting'];
			}
		...
		protected function set_active_callback() {
			...
			if ( ! empty( $this->required ) ) {
				$this->active_callback = array( 'Lesse_Kirki_Active_Callback', 'evaluate' );
				return;
			}
		...
		protected function set_sanitize_callback() {
			...
			$default_callbacks = array(
				'lessekirki-multicheck'       => array( 'Lesse_Kirki_Sanitize_Values', 'multicheck' ),
				'lessekirki-sortable'         => array( 'Lesse_Kirki_Sanitize_Values', 'sortable' ),
				'lessekirki-typography'       => array( 'Lesse_Kirki_Sanitize_Values', 'typography' ),
			);
		...
		protected function set_output() {
			...
			foreach ( $this->output as $output ) {
				...
				if ( ! isset( $output['property'] ) && ! in_array( $this->type, array( 'lessekirki-typography', 'background' ), true ) ) {
					continue;
				}
