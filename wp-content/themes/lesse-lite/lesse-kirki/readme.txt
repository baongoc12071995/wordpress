...
Donate link: http://lessekirki.org/
...
[![Build Status](https://travis-ci.org/aristath/lessekirki.svg?branch=master)](https://travis-ci.org/aristath/lessekirki) [![Scrutinizer Code Quality](https://scrutinizer-ci.com/g/aristath/lessekirki/badges/quality-score.png?b=master)](https://scrutinizer-ci.com/g/aristath/lessekirki/?branch=master) [![Code Climate](https://codeclimate.com/github/aristath/lessekirki/badges/gpa.svg)](https://codeclimate.com/github/aristath/lessekirki) [![License](https://img.shields.io/badge/license-GPL--2.0%2B-red.svg)](https://raw.githubusercontent.com/aristath/lessekirki/master/LICENSE) [![Join the chat at https://gitter.im/aristath/lessekirki](https://badges.gitter.im/Join%20Chat.svg)](https://gitter.im/aristath/lessekirki?utm_source=badge&utm_medium=badge&utm_campaign=pr-badge&utm_content=badge)

Lesse_Kirki is a Toolkit allowing WordPress developers to use the Customizer and take advantage of its advanced features and flexibility by abstracting the code and making it easier for everyone to create beautiful and meaningful user experiences.

Lesse_Kirki does not replace the WordPress Customizer API, you can still use the default WordPress methods and we advise you to familiarize yourselves with it. An excellent handbook for the WordPress Customizer can be found on the developer.wordpress.org website.

What Lesse_Kirki does is save you time… LOTS of time!
...
You can find detailed documentation on how to use Lesse_Kirki on [https://lessekirki.org](https://lessekirki.org)

== Installation ==

Simply install as a normal WordPress plugin and activate.

If you want to integrate Lesse_Kirki in your theme or plugin, please read the instructions on [our ducumentation site](https://lessekirki.org/docs/advanced/integration.html).
...
* FIX: Editor field covering the content ([#955](https://github.com/aristath/lessekirki/issues/955)).
* FIX: Smoother transition for editor switching.
* FIX: Code field JS error when using "php" mode ([#958](https://github.com/aristath/lessekirki/issues/958)).
* FIX: `postMessage` for typography fields ([#528](https://github.com/aristath/lessekirki/issues/528)).
* FIX: translation strings ([#960](https://github.com/aristath/lessekirki/issues/960)).
* FIX: `postMessage` for `background-image` properties ([#963](https://github.com/aristath/lessekirki/issues/963)).
* FIX: Reset Typography Control without font-family default value ([#951](https://github.com/aristath/lessekirki/issues/951)).
* FIX: Typography field: font-style missing in CSS output if variant is regular/400 ([#977](https://github.com/aristath/lessekirki/issues/977)).
* FIX: Placing two editor controls in the customizer leads to odd behavior ([#140](https://github.com/aristath/lessekirki/issues/140)).
* FIX: Typography field: letter-spacing missing in CSS output if its value is 0 ([#978](https://github.com/aristath/lessekirki/issues/978)).
* FIX: Allow using HTML in section descriptions ([#976](https://github.com/aristath/lessekirki/issues/976)).
* FIX: Bug preventing partial refreshes from working properly ([#991](https://github.com/aristath/lessekirki/issues/991)).
* FIX: Better internationalization handling.
* FIX: Output errors on typography settings ([#975](https://github.com/aristath/lessekirki/issues/975)).
* NEW: Added a new `attr` argument to `js_vars` ([#957](https://github.com/aristath/lessekirki/issues/957)).
* NEW: Implemented both `AND` and `OR` conditionals in `active_callback` arrays ([#839](https://github.com/aristath/lessekirki/issues/839)).
...
* NEW: Added a new `lessekirki/{$config_id}/styles` filter ([#908](https://github.com/aristath/lessekirki/issues/908)).
* NEW: Added a `customize-control-lessekirki` class to all Lesse_Kirki controls.
* FIX: Field type number : Cannot read property 'min' of undefined ([#911](https://github.com/aristath/lessekirki/issues/911)).
* FIX: All controls are now prefixed ([#918](https://github.com/aristath/lessekirki/issues/918))
* FIX: `alpha` argument in color-alpha controls ([#932](https://github.com/aristath/lessekirki/issues/932)).
...
* FIX: Placing 2 editor controls in the customizer leads to odd behaviour ([#140](https://github.com/aristath/lessekirki/issues/140)).
* FIX: `active_callback` conbined with the old `required` argument. ([#906](https://github.com/aristath/lessekirki/issues/906)).
* FIX: Double prefix and suffix in `js_vars` ([#943](https://github.com/aristath/lessekirki/issues/943)).
* FIX: Typography control returns both 'subset' and 'subsets' indexes with the same value ([#948](https://github.com/aristath/lessekirki/issues/948)).
...
Lesse_Kirki is now 100% WordPress Coding Standards compliant.
...
* FIX: Javascript Console Errors: "wp.customize" object undefined when Lesse_Kirki fields were added in `customize_register`
...
* FIX: Issue with URLs when using Lesse_Kirki embedded in a theme and not installed as a plugin.
...
* NEW: Started refactoring the `Lesse_Kirki_Field` class. Now each field can have its own sub-class extending the main Lesse_Kirki_Field object.
* NEW: `multicolor` control.
* NEW: Added `cropped_image` support in `repeater`. Props @guillaumemolter
* TWEAK: Renamed `Lesse_Kirki_Customizer_Scripts_Loading` to `Lesse_Kirki_Scripts_Loading`.
* TWEAK: Renamed `Lesse_Kirki_Customizer_Scripts_Tooltips` to `Lesse_Kirki_Scripts_Tooltips`.
* TWEAK: Renamed `Lesse_Kirki_Customizer_Scripts_Icons` to `Lesse_Kirki_Scripts_Icons`.
* TWEAK: More inline comments, docs & coding-standards improvements.
* DEPRECATED: Removed the `Lesse_Kirki_Colourlovers` class.
...
* FIX: Styles were being applied to the customizer even if they were not defined in the `lessekirki/config` filter.
...
* NEW: Added Material design palettes in the `Lesse_Kirki_Helper` class.
...
* NEW: Allow disabling the custom loader using the `disable_loader` argument in the `lessekirki/config` filter.
...
* FIX: PHP Notices in the class-lessekirki-output-control-typography.php file
* FIX: PHP Fatal error on PHP 5.2
* FIX: PHP Notice in the class-lessekirki-field.php file
...
* FIX: Typo in the textdomain for some strings (some strings were using "Lesse_Kirki" instead of "lessekirki").
...
* NEW: Deprecated the `Lesse_Kirki_Field_Sanitize` class in favor of a more simplified & robust implementation.
* NEW: Completely refactored the `Lesse_Kirki_Field` class, we're migrating to a more OOP model.
* NEW: Added a new `lessekirki-generic` control.
* NEW: Deprecated the custom text control and used the new `lessekirki-generic` control instead.
* NEW: Deprecated the custom textarea control and used the new `lessekirki-generic` control instead.
...
* FIX: Only load Lesse_Kirki styles when in the customizer
* FIX: Performance issue with Google Fonts
* NEW: Added radio-image controls to repeaters
* TWEAK: Better color handling in the Lesse_Kirki_Color class
...
* TWEAK: Removed demo theme from the plugin. This is now provided separately on https://github.com/aristath/lessekirki-demo
...
* FIX: Minor bugfixes to the Lesse_Kirki_Color class.
* FIX: lessekirki_get_option now uses Lesse_Kirki::get_option().
...
* NEW: Added a new API. See https://github.com/aristath/lessekirki/wiki for documentation.
* NEW: Minimum PHP requirement is now PHP 5.2
* NEW: Added a Select2 field type.
* NEW: Introducing the Lesse_Kirki::get_option() method to get values.
* NEW: added 'media_query' argument to output.
* NEW: Added ability to get variables for CSS preprocessors from the customizer values. See https://github.com/aristath/lessekirki/wiki/variables for documentation
* NEW: now supporting 'units' to all outputs to support '!important'
* NEW: Ability to create panels & sections using the new API.
* NEW: added a get_posts method to the Lesse_Kirki class.
* NEW: Implement width argument in the styling options. See https://github.com/aristath/lessekirki/wiki/Styling-the-Customizer
* NEW: add 'lessekirki/control_types' filter
...
* FIX: `Lesse_Kirki::get_option()` method now works for all fields, including background fields.
...
* TWEAK: Improve the Lesse_Kirki_Styles_Customizer class
* TWEAK: Code cleanups
...
* TWEAK: Better color manipulation in the Lesse_Kirki_Color class
* TWEAK: Move secondary classes instantiation to the Lesse_Kirki() function.
* TWEAK: set a $lessekirki global
* TWEAK: deprecate getOrThrow method in the Lesse_Kirki_Config class.
* TWEAK: Move sanitisation functions to a Lesse_Kirki_Sanitize class.
* TWEAK: Rename Lesse_Kirki_Framework to Lesse_Kirki_Toolkit.
* TWEAK: Move variables to the new API
* TWEAK: simplify Lesse_Kirki_Controls class
* TWEAK: move the lessekirki/fields & lessekirki/controls filters to the new API
...
* FIX: Issues with the URL when Lesse_Kirki is embedded in a theme
...
* NEW: Added a Lesse_Kirki_Colourlovers class to use palettes from the colourlovers API
...
* NEW: added a functional lessekirki_get_option() function
* TWEAK: Simplified configuration options.
* NEW: Turn Lesse_Kirki into a singleton and a facade (props @vpratfr)
...
* REMOVED: Remove the `lessekirki_get_option` function that was introduced in 0.7 as it's not working properly yet.
...
* NEW: Added `description` argument in the lessekirki configuration (replaces the theme description)
...
* NEW: Added `lessekirki_get_option( $setting );` function that also gets default values
...
* NEW: Introducing Lesse_Kirki_Customize_Control class
* FIX: CSS tweaks
...
* NEW: Change the Lesse_Kirki theme based on which admin theme is selected.
