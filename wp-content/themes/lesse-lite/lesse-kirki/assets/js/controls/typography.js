wp.customize.controlConstructor['lessekirki-typography'] = wp.customize.Control.extend({
	...
	ready: function() {
		...
		renderSubControl = function( fontFamily, sub, startValue ) {
			...
			_.each( lessekirkiAllFonts, function( font, i ) {
			...
			if ( true === isStandard ) {
				...
				if ( 2 > subList.length ) {
					...
					control.container.find( '.lessekirki-' + sub + '-wrapper' ).css( 'display', 'none' );
				} else {
					control.container.find( '.lessekirki-' + sub + '-wrapper' ).css( 'display', 'block' );
				}
		...
		picker = this.container.find( '.lessekirki-color-control' );
