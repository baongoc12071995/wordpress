...
wp.customize.controlConstructor.repeater = wp.customize.Control.extend({
	ready: function() {
		...
		// The current value set in Control Class (set in Lesse_Kirki_Customize_Repeater_Control::to_json() function)
		...
		this.container.on( 'keypress, keyup', '.lesse-autocomplete', function( e ) {
			if ( ! jQuery.trim( jQuery(this).val() ) ) {
				jQuery( this ).closest( '.repeater-row-content' ).find( 'input[type="hidden"]' ).val( '' ).trigger( 'change' );
				jQuery( this ).trigger( 'change' );
				wp.customize.previewer.refresh();
			}
		});

		control.createAutocomplete( control );
	...
	createAutocomplete: function( control ) {

		if ( ajaxurl === undefined ) {
			return false;
		}

		jQuery( ".lesse-autocomplete" ).not( '.ui-autocomplete-input' ).each( function() {
			var $this       = jQuery(this),
			    _this       = this,
			    ajaxAction  = 'lesse_autocomplete_search';
			...
			$this.autocomplete({
				...
				source: function ( request, response ) {
					jQuery.ajax ({
						...
						data: {
							'action': ajaxAction,
							'term': request.term,
							'security': lesseKirkiparams.nonce
						},
				...
				search: function( event, ui ) {
					$this.closest( 'label' ).find( '.spinner' ).css( 'visibility', 'visible' );
				},

				response: function( event, ui ) {
					$this.closest( 'label' ).find( '.spinner' ).css( 'visibility', 'hidden' );
				},

				select: function (event, ui) {
					if ( jQuery.isEmptyObject (ui) || ui.item === undefined ) {
						return false;
					}
					_.delay( function(){
						var $hiddenField = $this.closest( '.repeater-row-content' ).find( '.repeater-field-hidden input' );
						$this.trigger( 'change' );
						$hiddenField.val( ui.item.post_id ).trigger( 'change' );
					}, 200 );
				}
			});
		});
	...
	setImageInRepeaterField: function( attachment ) {
		...
		$targetDiv.find( '.lessekirki-image-attachment' ).html( '<img src="' + attachment.url + '">' ).hide().slideDown( 'slow' );
	...
	setFileInRepeaterField: function( attachment ) {
		...
		$targetDiv.find( '.lessekirki-file-attachment' ).html( '<span class="file"><span class="dashicons dashicons-media-default"></span> ' + attachment.filename + '</span>' ).hide().slideDown( 'slow' );
	...
	removeImage: function( event ) {
		...
		$targetDiv.find( '.lessekirki-image-attachment' ).slideUp( 'fast', function() {
	...
	removeFile: function( event ) {
		...
		$targetDiv.find( '.lessekirki-file-attachment' ).slideUp( 'fast', function() {
			jQuery( this ).show().html( jQuery( this ).data( 'placeholder' ) );
		});
		$targetDiv.find( '.hidden-field' ).val( '' );
		$uploadButton.text( $uploadButton.data( 'label' ) );
	...
	addRow: function( data ) {
		...
		var control       = this,
		    template      = control.repeaterTemplate(), // The template for the new row (defined on Lesse_Kirki_Customize_Repeater_Control::render_content() ).
